...
Blockly.Python['lists_append'] = function(block) {
  ...
  var code = variable_list + '.append(' + value_input.trim() + ')\n';
  return code;
};
...
Blockly.Python['lists_length'] = function(block) {
  ...
  return ['len(' + list.trim() + ')', Blockly.Python.ORDER_FUNCTION_CALL];
};
  ...
  if (block.getFieldValue('END') == 'FIRST') {
    var functionName = Blockly.Python.provideFunction_(
        ...
        ['def ' + Blockly.Python.FUNCTION_NAME_PLACEHOLDER_ +
         ...
         '  return index']);
    var code = functionName + '(' + list.trim() + ', ' + item + ')';
...
Blockly.Python['lists_getIndex'] = function(block) {
  ...
  switch (where) {
    case 'FIRST':
      if (mode == 'GET') {
        var code = list.trim() + '[0]';
        return [code, Blockly.Python.ORDER_MEMBER];
      } else if (mode == 'GET_REMOVE') {
        var code = list.trim() + '.pop(0)';
        return [code, Blockly.Python.ORDER_FUNCTION_CALL];
      } else if (mode == 'REMOVE') {
        return list.trim() + '.pop(0)\n';
      }
      break;
    case 'LAST':
      if (mode == 'GET') {
        var code = list.trim() + '[-1]';
        return [code, Blockly.Python.ORDER_MEMBER];
      } else if (mode == 'GET_REMOVE') {
        var code = list.trim() + '.pop()';
        return [code, Blockly.Python.ORDER_FUNCTION_CALL];
      } else if (mode == 'REMOVE') {
        return list.trim() + '.pop()\n';
      }
      break;
    case 'FROM_START':
      var at = Blockly.Python.getAdjustedInt(block, 'AT');
      if (mode == 'GET') {
        var code = list.trim() + '[' + at + ']';
        return [code, Blockly.Python.ORDER_MEMBER];
      } else if (mode == 'GET_REMOVE') {
        var code = list.trim() + '.pop(' + at + ')';
        return [code, Blockly.Python.ORDER_FUNCTION_CALL];
      } else if (mode == 'REMOVE') {
        return list.trim() + '.pop(' + at + ')\n';
      }
      break;
    case'FROM_END':
...
Blockly.Python['lists_split'] = function(block) {
  ...
  if (mode == 'SPLIT') {
    ...
    var value_delim = Blockly.Python.valueToCode(block, 'DELIM',
        Blockly.Python.ORDER_NONE);
    var code = value_input.trim() + '.split(' + value_delim + ')';
  } else if (mode == 'JOIN') {
    var value_input = Blockly.Python.valueToCode(block, 'INPUT',
        Blockly.Python.ORDER_NONE) || '[]';
    var value_delim = Blockly.Python.valueToCode(block, 'DELIM',
        Blockly.Python.ORDER_MEMBER) || '\'\'';
    var code = value_delim + '.join(' + value_input.trim() + ')';
  } else {
...
Blockly.Python['lists_reverse'] = function(block) {
  ...
  var list = Blockly.Python.valueToCode(block, 'LIST',
      Blockly.Python.ORDER_NONE) || '[]';
  var code = 'list(reversed(' + list.trim() + '))';
  return [code, Blockly.Python.ORDER_FUNCTION_CALL];
};
